...
DB_PATH = os.path.join(PROJECT_ROOT, "data", "telemetry.db")

MISFIRE_DROP = 800.0
MISFIRE_SPEED_MIN = 5.0
...
def load_samples():
    ...
    rows = run_query(
        """
        SELECT timestamp, source, rpm, speed, coolant_temp, oil_temp, boost, voltage
        FROM telemetry
        ORDER BY id ASC
        """
    )
...
def show_max_and_average():
    print("\n--- Max / Average Values ---")
    max_rpm = run_query("SELECT MAX(rpm) FROM telemetry")[0][0]
    max_coolant = run_query("SELECT MAX(coolant_temp) FROM telemetry")[0][0]
    max_oil = run_query("SELECT MAX(oil_temp) FROM telemetry")[0][0]
    max_boost = run_query("SELECT MAX(boost) FROM telemetry")[0][0]
    min_voltage = run_query("SELECT MIN(voltage) FROM telemetry")[0][0]
    avg_voltage = run_query("SELECT AVG(voltage) FROM telemetry")[0][0]

    if None in (max_rpm, max_coolant, max_oil, max_boost, min_voltage, avg_voltage):
...
def detect_anomalies(samples):
    ...
    rpm_high_events = [s for s in samples if s["rpm"] >= RPM_HIGH]
    if rpm_high_events:
        print(f"\nHigh RPM events (>={RPM_HIGH:.0f} rpm): {len(rpm_high_events)}")
    else:
        print(f"\nNo high RPM events (>={RPM_HIGH:.0f} rpm) detected.")
...
def analyze_heat_soak(samples):
    ...
    dt = (
        parse_timestamp(last["ts"])
        - parse_timestamp(first["ts"])
    ).total_seconds()
...
def plot_time_series(samples):
    ...
    times = [parse_timestamp(s["ts"]) for s in samples]
    t0 = times[0]
...
def main():
    ...
    if not samples:
        print("No telemetry data found in data/telemetry.db yet.")
        return
